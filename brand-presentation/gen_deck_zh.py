#!/usr/bin/env python3
# 迦拿 · 加利利 — 品牌与产品系列 (Chinese). Palette-forward, matching the EN deck. 和合本 verses, kinsoku.
import os, re
...
for nm in ["Cormorant-Medium","Cormorant-Regular","Cormorant-Italic","WorkSans-Regular","WorkSans-Medium",
...
LINE=HexColor("#D8CCB4"); CARD=HexColor("#FBF7EF")
W,H=960,540; M=58
...
def wrap_zh(s,font,size,maxw):
    # tokenize so Latin/number runs (e.g. "Work Sans", "350") never break mid-word
    toks=re.findall(r'[A-Za-z0-9@._\-:/]+|\s+|[^\sA-Za-z0-9@._\-:/]', s)
    lines=[]; cur=""
    for tk in toks:
        if not cur.strip() or c.stringWidth(cur+tk,font,size)<=maxw: cur+=tk
        elif tk in _CLOSE: cur+=tk; lines.append(cur.strip()); cur=""
        else: lines.append(cur.strip()); cur=("" if tk.isspace() else tk)
    if cur.strip(): lines.append(cur.strip())
    return lines
def para(t,x,top,w,style="body",color=None):
    font,size,leading,col=STYLES[style]; col=color or col; t=t.replace("<b>","").replace("</b>","")
    lines=wrap_zh(t,font,size,w)
    for i,ln in enumerate(lines): T(x,top+size+i*leading,ln,font,size,col)
...
def wordmark(cx,top,cana=52,sub=17,col=UMBER,rc=OLIVE,sc=None):
    ...
    T(cx,top+cana+34,"迦拿 · 加利利",ZS_R,sub,sc,align="center",track=sub*0.5)

def footer(n,x0=M):
    rule(x0,512,W-M-x0,LINE,0.7)
    T(x0,525,"CANA · GALILEE — 品牌与产品系列",ZH_M,7.5,MUT,track=1.2); T(W-M,525,f"{n:02d}",ZH_M,7.5,MUT)
...
coin(150,120,80)
kicker(M,196,"品牌与产品系列 · 2026",col=GOLD)
wordmark(M+150,222,cana=58,sub=19,col=BONE,rc=GOLD,sc=LINEN)
T(M,360,"为基督徒的家，制作可以传世的陶瓷。",ZH_R,12,LINEN)
T(M,450,"「我们有这宝贝放在瓦器里。」",ZS_R,15,BONE)
T(M,470,"哥林多后书 4:7",ZH_M,8.5,GOLD,track=1.5)
c.showPage()

# ===================================================== 2 — BRAND
page()
img_cover(PUB+"/hands.png", 0,0, 452, H)
kickrule(512,150,"品牌",rc=GOLD)
T(512,196,"可以捧在手中的圣言。",ZS_SB,31,UMBER)
para("迦拿为基督徒的家，制作可以传世的陶瓷——温润的象牙色炻器，将一句圣经经文刻入陶土，让圣言成为能握在手心的器物。",512,232,W-512-M,"lead")
para("品牌之名取自「加利利的迦拿」——耶稣行第一个神迹之地；那里有筵席、有款待，是平凡被圣化之处。每一件皆以手工制成，只为被珍藏、被传承。我们雕刻，而非印刷；我们祝福，而不说教。",512,300,W-512-M,"lead")
footer(2,508)
c.showPage()

# ===================================================== 3 — PALETTE / SYSTEM
page()
kickrule(M,86,"品牌系统",rc=GOLD)
T(M,126,"陶土、亚麻、橄榄、烛光。",ZS_SB,32,UMBER)
sw=[("骨白","#F4EDDE",BONE,"基底"),("亚麻","#E7DAC4",LINEN,"包装"),("棕土","#54442F",UMBER,"印记"),
    ("浓咖","#3A2E20",ESP,"正文"),("橄榄","#7C7A53",OLIVE,"橄榄枝"),("古金","#B5934E",GOLD,"微光 · 烫金"),
    ("赤陶","#B5683F",TERRA,"季节点缀")]
gap=13; sww=(W-2*M-(len(sw)-1)*gap)/len(sw); sy=176; sh=192
for i,(nm,hx,col,role) in enumerate(sw):
    ...
    T(x,sy+sh+20,nm,ZS_SB,13,UMBER); T(x,sy+sh+35,hx,"WorkSans-Regular",8,MUT); T(x,sy+sh+48,role,ZH_R,7.5,MUT)
para("约七成骨白与亚麻为底，两成棕土与浓咖为字，橄榄与金色点到为止——克制，才显高级。字体：思源宋体（标题与经文）· 思源黑体（正文），西文以 Cormorant Garamond 与 Work Sans 搭配。",M,sy+sh+68,W-2*M,"body")
footer(3)
c.showPage()

# ===================================================== 4 — COLLECTION
page()
kickrule(M,86,"产品系列",rc=GOLD)
...
page(ESP)
coin(W/2,150,80)
T(W/2,308,"我们有这宝贝",ZS_R,40,BONE,align="center")
T(W/2,362,"放在瓦器里。",ZS_R,40,BONE,align="center")
rule(W/2-42,392,84,GOLD,1)
T(W/2,420,"哥林多后书 4:7 · 品牌核心经文",ZH_M,8.5,GOLD,align="center",track=2)
c.showPage()

# ===================================================== 10 — CRAFT
page()
kickrule(M,86,"工艺与材质",rc=GOLD)
...
tx=M+266+252+34
para("经文皆刻入陶土，绝不印刷——同色同质，每件器物使用同一种字体；让阴影替你诵读，也让整个系列宛如出自同一双手。",tx,176,W-tx-M,"body")
pts=["温润的象牙／骨白色炻器——并非惨白","哑光至缎光釉面；底部素坯，触感质朴",
     "件件手工——绝无两件完全相同","简约恒久的造型——话语，即是唯一的装饰"]
for i,t in enumerate(pts):
    yb=300+i*34; T(tx,yb,"—",ZH_R,10,GOLD); para(t,tx+16,yb-12,W-tx-M-16,"small")
footer(10)
c.showPage()

# ===================================================== 11 — PACKAGING
page()
kickrule(M,86,"包装与礼物",rc=GOLD)
...
for i,im in enumerate([FP+"/mug/mug-box-closed.png",FP+"/giftset/giftset-box-open.png",FP+"/mug/mug-box-below-newpanel.png"]):
    x=M+i*(pw+20); img_cover(im,x,168,pw,244); c.setStrokeColor(LINE); c.setLineWidth(0.8); c.rect(x,H-168-244,pw,244,fill=0,stroke=1)
para("骨白与亚麻色的硬质礼盒，盒盖压印橄榄枝，并以品牌印章封缄；内衬印有橄榄纹样的棉纸；每件器物皆附一张祝福卡——一面是经文，一面是它的含义。不晃动，不廉价。",M,440,W-2*M,"lead")
footer(11)
c.showPage()

# ===================================================== 12–13 — MADE TO SPEC
page()
kickrule(M,82,"制作规范",rc=GOLD)
...
para("一个实例——将礼盒重新构想为节省空间的叠装：碟平放于盒底，一层硬质刀模托架横跨其上，马克杯侧卧于凹槽之中，稳而不滚。",M,150,292,"lead")
...
page(ESP)
coin(W/2,182,94)
wordmark(W/2,250,cana=46,sub=16,col=BONE,rc=GOLD,sc=LINEN)
T(W/2,360,"为基督徒的家，制作可以传世的陶瓷。",ZH_R,12,LINEN,align="center")
T(W/2,400,"cana-galilee.com      ·      valentinpueschel@gmail.com","WorkSans-Regular",11,BONE,align="center")
T(W/2,452,"「我们有这宝贝放在瓦器里。」",ZS_R,15,BONE,align="center")
T(W/2,472,"哥林多后书 4:7",ZH_M,8.5,GOLD,align="center",track=1.5)
c.showPage()
